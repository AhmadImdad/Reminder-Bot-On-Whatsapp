...
def render():
    st.header("📓 Notes Store")
    st.caption("All notes captured from WhatsApp, newest first.")

    search = st.text_input("🔍 Search by subject...", key="note_search")
    df = database_queries.get_all_notes(search=search)

    if df.empty:
        if search:
            st.warning(f"No notes found matching **'{search}'**.")
        else:
            st.info(
                "📓 No notes saved yet!\n\n"
                "Send a WhatsApp message to your bot where the last sentence "
                "contains the word **'note'** to save your first one."
            )
        return
    ...
    with col2:
        st.metric("Notes with Media", with_media)
    ...
    for _, row in df.iterrows():
        ...
        with st.container(border=True):
            ...
            extra_attachments = database_queries.get_attachments("note", note_id)
            ...
            if total_media > 0:
                with st.expander(f"📎 View {total_media} attachment(s)", expanded=False):
                    if has_primary:
                        ...
                        render_media_item(media_type, _abs(media_path), original_name, key=f"note_primary_{note_id}")
                    if extra_attachments:
                        ...
                        for i, att in enumerate(extra_attachments):
                            render_media_item(
                                ...
                                key=f"note_att_{note_id}_{i}"
                            )

            if st.button("🗑️ Delete", key=f"del_note_{note_id}"):
                database_queries.delete_note_by_id(note_id)
                st.error(f"Note #{note_id} deleted.")
                st.rerun()
