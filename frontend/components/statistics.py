...
def render():
    ...
    tab1, tab2 = st.tabs(["⏰ Reminders", "📝 Tasks"])
    ...
    with tab2:
        stats_t = database_queries.get_task_stats()
        
        col1, col2, col3 = st.columns(3)
        col1.metric("Total Tasks Created", stats_t.get('total', 0))
        col2.metric("Tasks Completed", stats_t.get('completed', 0))
        col3.metric("Active Tasks", stats_t.get('pending', 0))
        ...
        with colA:
            ...
            status_df_t = pd.DataFrame({
                'Status': list(stats_t.keys()),
                'Count': list(stats_t.values())
            })
            status_df_t = status_df_t[status_df_t['Status'] != 'total']
            
            if not status_df_t.empty and status_df_t['Count'].sum() > 0:
                st.bar_chart(status_df_t.set_index('Status'), color="#28a745")
            else:
                st.info("Not enough data.")
                
        with colB:
            st.subheader("Creation Trends (Last 30 Days)")
            df_history_t = database_queries.get_task_history(days=30)
            
            if not df_history_t.empty:
                df_history_t['date'] = df_history_t['created_at'].dt.date
                counts_t = df_history_t.groupby('date').size()
                st.line_chart(counts_t, color="#dc3545")
            else:
                st.info("Not enough data.")
    ...
    h2.info(f"📨 **WhatsApp Messages Processed:** {msg_count}")
